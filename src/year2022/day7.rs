...
impl Day<2022,7,Node<Entry>,u64> for Day7
{
    ...
    fn parse(input: &str) -> Node<Entry> {
        ...
        let mut commands = input.lines().map(str::parse::<ConsoleCommand>).filter_map(Result::ok);
...
fn tree_size(node : Node<Entry>)
{
    ...
    {
        gmut.value.size = Some(gmut.children.iter_mut().map(|mut c| {
