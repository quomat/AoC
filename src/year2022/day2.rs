...
for Day2
{
    ...
    fn parse(input : String) ->  Vec<(Figure,Figure)>
    {
        parse_by_variant(input,false)
    }

    
    fn parse2(input : String) ->  Vec<(Figure,Figure)>
    {
...
fn parse_by_variant(input : String, variant : bool) -> Vec<(Figure,Figure)>
{
    let v = input.split('\n');
    v.filter_map(|s| 
        {
            let a = s.chars().nth(0).map(parse_enemy);
            let b = a.and_then(|enemy| s.chars().nth(2).map(|x| if variant {parse_own2(enemy,x)} else {parse_own(x)}));
            a.zip(b)
        }).collect()
}
