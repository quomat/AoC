...
impl Day<2022,8,Vec<Vec<u32>>,usize> for Day8
{
    fn solve(input: Vec<Vec<u32>>) -> usize {
        ...
        4*(n-1) + visible.into_iter().map(|v| v.into_iter().filter(|&x| x).count()).sum::<usize>()
    }
    ...
    fn solve2(input: Vec<Vec<u32>>) -> usize {
        ...
        (1..n-1).map(|k| (1..n-1).map(|m| scenic_score(&input, k, m)).max().unwrap()).max().unwrap()
    }
...
fn scenic_score(forest : &Vec<Vec<u32>> , i : usize, j : usize) -> usize
{
    ...
    let rays  : Vec<Box<dyn Fn(i32, i32) -> (i32, i32)>> = vec![
        ...
        Box::new(move |i,j| (i,j+1))    ];
        rays.iter().map(|f| raycast(f,forest,i,j)).fold(1,|x,y| x*y)
}
